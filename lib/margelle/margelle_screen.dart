...
import 'manage_margelle_widget.dart';
...
class _MargelleScreenState extends State<MargelleScreen> {
  ...
  void initState() {
    ...
    if (widget.margelleData != null) {
      ...
      try {
        final item = widget.margelleData!.mesureDeMargelle as List<dynamic>;
        item.forEach((element) {
          margelleCount.add(element.toString());
        });
        log("kkk.....     ${item}");
      } catch (e) {}
      setState(() {});

      // selectedValues = List.generate(widget.margelleData!.mesureDeMargelle.length, (index) => null);
      //
      // setState(() {
      //   int? number = int.tryParse(widget.margelleData!.mesureDeMargelle.length.toString());
      //   if (number != null) {
      //     for (int i = 0; i < number; i++) {
      //       _textList.add(i.toString());
      //       autreController.add(TextEditingController());
      //
      //       selectedValues.add(PositionItem(id: i, name: widget.margelleData!.mesureDeMargelle[i]));
      //       log("jkhjkgjhg" + selectedValues.toList().toString());
      //     }
      //
      //     // if (selectedValues.length < _textList.length) {
      //     //   int diff = _textList.length - selectedValues.length;
      //     //   for (int i = 0; i < diff; i++) {
      //     //     selectedValues.add(widget.margelleData!.mesureDeMargelle[i]);
      //     //
      //     //     log("jkhjkgjhg"+selectedValues.toList().toString());
      //     //   }
      //     // }
    ...
    //   log("jkhjk2222222${item?.name.toString()}");
    // }
  }
                  ...
                  child: Column(
                    children: [
                      ...
                      RegisterTextFieldWidget(
                        ...
                        onChanged: (value) {
                          margelleCount = [];
                          int? kk = int.tryParse(value);
                          if (kk == null) {
                            margelleCount = [];
                            setState(() {});
                            return;
                          }
                          margelleCount = List.generate(kk, (index) => "");
                          setState(() {});
                          // setState(() {
                          //   _textList.clear();
                          //   if (value.isNotEmpty) {
                      ...
                      ...List.generate(
                          margelleCount.length,
                          (index) => MargelleWidget(
                            index: index+1,
                                initial: margelleCount[index],
                                onUpdate: (String data) {
                                  margelleCount[index] = data;
                                  log("kkkkkk....    ${margelleCount[index]}");
                                  log("kkkkkk....    ${margelleCount}");
                                },
                              )),
                      // ...margelleCount.map((e) => MargelleWidget(
                      //   initial: e,
                      //   onUpdate: (String data) {
                      //   e = data;
                      //   log("kkkkkk....    ${e}");
                          ...
                          child: Obx(() {
                            if (categoryFile.value.path == "") {
                              if (images.value.isNotEmpty) {
                                ...
                                return widget.margelleData != null && widget.margelleData!.photoVideo != null
                                    ? Container(
                              ...
                              onPressed: () async {
                                ...
                                if (_formKey.currentState!.validate()) {
                                  ...
                                  Map<String, String> mapData = {
                                    ...
                                    'nombre_de_fenetre': widget.margelleData!.nombre_de_fenetre.toString(),
                                    "note": noteController.text,
                                    // "mesure_de_margelle[]": mesuredemargelleselectedValue!.name,
                                  };

                                  margelleCount.asMap().entries.forEach((element) {
                                    mapData["mesure_de_margelle[${element.key + 1}]"] = element.value;
                                  });
                              ...
                              onPressed: () async {
                                if (_formKey.currentState!.validate()) {
                                  ...
                                  Map<String, dynamic> mapData = {
                                    "client_id": widget.clientId.toString(),
                                    'nombre_de_fenetre': nombredefenetreController.text,
                                    "note": noteController.text,
                                  };
                                  margelleCount.asMap().entries.forEach((element) {
                                    mapData["mesure_de_margelle[${element.key + 1}]"] = element.value;
                                  ...
                                  MargelleScreenRepo.margelleScreenRepo(
                                          context: context,
                                          mapData: mapData.map((key, value) => MapEntry(key, value.toString())),
                                          fieldName1: 'photo_video[]',
                                          files: images.value)
                                      .then((value) {
  ...
  //                         }
  //                       }
  //                       log(selectedItemsList.map((e) => e.name).toList().toString());
  //                     });
  //                   },
